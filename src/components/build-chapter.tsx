...
export function BuildChapter({ ref }: { ref: Ref<BuildChapterHandle> }) {
  ...
  useImperativeHandle(ref, () => ({
    update(progress, visible = true) {
      ...
      const serviceReveal = phase(progress, .2, .28);
      ...
      if (services.current) {
        ...
        services.current.querySelectorAll<HTMLElement>("[data-service-copy]").forEach((card, i) => {
          ...
          const leave = i === 2 ? 0 : phase(progress, end - .04, end);
    ...
    reset() {
      ...
      [identity.current, services.current].forEach(element => { if (element) { element.inert = false; element.removeAttribute("aria-hidden"); } });
  ...
  return <div className={styles.chapter} ref={root}>
    ...
    <div className={styles.services} ref={services}>
      ...
      <div className={styles.serviceCopy}>
        <article data-service-copy><p>01 / IMAGINE IT. ENGINEER IT.</p><h2 id="services-title">Your business.<br /><em>Your way of working.</em></h2><p>Custom software shaped around your idea, or a proven system made yours. Websites, applications and CRMs that fit the way your business works.</p><small>CUSTOM SOFTWARE / WEB APPS / CRM</small></article>
